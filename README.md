...
### 14.1 第4段ペイロードの実物を取得できていない

`.cofigs/.google_service_config.jar` の存在と実行は dexopt 記録から証明したが、ファイル本体は取得できなかった。全経路を検証した結果:

| 経路 | 結果 |
|---|---|
| ADB shell 直接読み取り | 不可(`/data/data/<pkg>` は 0700、アプリ所有) |
| `adb backup` | 不可。launcher3 は `<include>` 3件限定のバックアップ規則、vending/chrome は key/value エージェント方式、packageinstaller は `allowBackup=false` |
| userdata 生ダンプ | 不可。FBE(`aes-256-xts:aes-256-cts:v2` + メタデータ暗号化)、鍵は TEE 内 |
| ブートローダアンロックで root | 逆効果。アンロックは `/data` を消去し、取得対象そのものが失われる |

内容については公開解析(第三者の逆コンパイル結果)により判明している(§3.3)が、**自らバイナリを検証したものではない。**

### 14.2 C2 配信の最終モジュールは未解析

公開解析には、C2 が実際に配信したモジュール `com.hduu.wutiu.CkUtils`(`cfiles.f4dc6d8n.com` から配信、7,904 B)の暗号化ファイルが含まれている。復元済みの定数(AES 鍵候補、XOR 鍵、ZIP パスワード候補)を総当たりしたが復号できなかった。**本調査でも公開解析でも、この最終モジュールが何をするかは未解明である。**

### 14.3 本端末で実際に何が送信されたかは未確認

`/data` パーティションはダンプに含まれず、実機の非公開領域にも到達できないため、C2 とのやり取りの実測はできていない。C2 稼働窓(端末セットアップ 2026-07-28 19:17 〜 ドメイン停止 2026-07-29 21:16、約26時間)に何が起きたかは、dexopt 記録から「第4段を受信して実行した」ことまでしか言えない。

傍証として、通常ほぼ外向き通信を行わない `com.android.launcher3`(UID 10119)に、この窓の内側にあたる 2026-07-29 19:00〜21:00 のバケットで送信 4,750 バイト / 29 パケットが記録されているが、2時間幅のバケットであり単独では決定的でない。

### 14.4 未解決の関係

- Redstone / otauc の2系統のサイレントインストール経路とインプラントの関係(共犯か単なる同居か)
- `M9FNY5PNCx2ZwJam` が実際にドメインリスト復号鍵であるかの検証(SharedPreferences に到達できないため未確認)
- YRCT の実体

### 14.5 実施可能な追加調査

**制御ネットワークでの通信捕捉** — インプラントは TLS 検証を無効化しているため自己署名証明書で傍受できる。隔離 Wi-Fi(インターネット経路なし)+ DNS で C2 ドメインを検査機に解決 + ポート10129 で HTTPS 待受により、実際のチェックイン内容を実測できる。ただし送信間隔はサーバ応答の `nextRequestTime` 依存で、コード中には3日のゲートもあるため即座に発報するとは限らない。

**登録一括性を起点とした横展開** — NameCheap で 2025-06-19 06:54:31 に登録され Cloudflare NS `sri`/`suzanne` を使うドメイン群から、未知の兄弟ドメインを列挙できる。

### 14.6 調査による状態変化

- 実機調査中に `logcat -c` を1度実行しログバッファをクリアした。クリア前の内容は本報告に採録済みだが、それ以前の履歴の一部は失われた
- bugreport の生成以外、端末への書き込み・設定変更は行っていない
- 端末は調査中も稼働を継続しており、新規プロセスへの展開が進行している。証拠状態を固定するには電源を切る必要がある

---

## 15. 調査中の判断訂正の記録

透明性のため、調査の進行に伴って訂正した判断を記録する。

| 時点 | 当初の判断 | 訂正後 | 訂正の根拠 |
|---|---|---|---|
| 静的解析段階 | 第3段Bは SMS 操作が可能 | **SMS の送受信・読み取り機能はない** | `SmsManager` の用途はデュアル SIM の既定スロット判定のみ。`sendTextMessage` / `content://sms` への参照ゼロ |
| 実機調査前 | C2 停止により現在の活動は止まっている | **インプラント本体は常時稼働し、起動するプロセスに展開を続けている** | ブロードキャストが1分9秒前に発報。`package-dex-usage.list` が調査10分前に更新 |
| 実機調査中盤 | C2 からタスクを受信した形跡はない | **受信して実行済み** | dexopt 記録に第4段の読み込み履歴、および ART 生成の `.vdex` |
| リスク評価 | 機能の上限は測れない | 本端末に配信された第4段については測れた(C2 冗長化モジュールであり新規攻撃機能ではない) | 公開解析による第4段の内容判明 |

---

*本報告書は 2026-07-31 時点の調査結果に基づく。*
*調査は元ダンプのコピーおよび実機の読み取り専用アクセスにより実施し、`XS19Pro_dump/` は一切変更していない。*
